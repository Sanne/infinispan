...
class DirectoryImplementor {
    ...
    public DirectoryImplementor(Cache<?, ?> metadataCache, Cache<?, ?> chunksCache, String indexName, int chunkSize, SegmentReadLocker readLocker, boolean fileListUpdatedAsync, int affinitySegmentId) {
      ...
      if (chunkSize <= 0)
        ...
        this.fileOps = new FileListOperations(this.metadataCache, indexName, fileListUpdatedAsync, affinitySegmentId);
        segmentsGenFileKey = new FileCacheKey(indexName, IndexFileNames.SEGMENTS_GEN, affinitySegmentId);
